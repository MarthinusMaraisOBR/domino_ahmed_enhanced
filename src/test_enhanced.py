#!/usr/bin/env python3
"""
FIXED Enhanced DoMINO model testing with proper input normalization.
This version correctly normalizes inputs and unnormalizes outputs.
"""
    ...
    with torch.no_grad():
        ...
        if prediction_surf is not None and surf_factors is not None:
            ...
            if surf_factors.shape[1] == 8:
                # Use FINE statistics (first 4 columns) for output unnormalization
            ...
            # Debug: Check raw model output before unnormalization
            ...
            # Unnormalize with fine statistics
...
def main(cfg: DictConfig):
    print("="*80)
    print("ENHANCED DoMINO MODEL TESTING - WITH PROPER NORMALIZATION")
    print("="*80)
    ...
    # Load scaling factors - ensure we load the enhanced version
    surf_save_path = os.path.join(
        cfg.eval.scaling_param_path, "scaling_factors_enhanced.npy"  # CHANGED to enhanced
    )
    ...
    for count, dirname in enumerate(coarse_test_cases[:num_cases]):
        ...
        try:
            ...
            data_dict = {
                ...
                "surface_fields": normalized_coarse_fields,  # CHANGED: Use normalized fields
            ...
            if prediction_surf is not None:
                ...
                save_comprehensive_vtp(
                    ...
                    coarse_fields=coarse_data['fields'],  # Save original unnormalized for visualization
