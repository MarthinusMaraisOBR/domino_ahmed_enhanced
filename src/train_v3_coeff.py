...
from coefficient_data_loader import EnhancedDatasetWithCoefficients, CoefficientManager
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        with autocast(enabled=True):
            ...
            if use_physics_loss:
                ...
                if cfg.model.get('use_multitask_loss', False):
                    ...
                    # Get coefficient targets if available
                    coeff_targets = sampled_batched.get('fine_coefficients', None)
                    
                    # Get coefficient predictions from model
...
def main(cfg: DictConfig) -> None:
    ...
    # Create datasets - NOW WITH COEFFICIENT SUPPORT
    ...
    if use_coefficient_data:
        ...
        # Use the enhanced dataset with coefficients
        ...
        if hasattr(train_dataset, 'coeff_stats'):
            stats = train_dataset.coeff_stats
            logger.info("  Training coefficient statistics:")
            logger.info(f"    Fine Cd: mean={stats['fine_cd_mean']:.4f}, std={stats['fine_cd_std']:.4f}")
            logger.info(f"    Fine Cl: mean={stats['fine_cl_mean']:.4f}, std={stats['fine_cl_std']:.4f}")
            logger.info(f"    Coarse Cd: mean={stats['coarse_cd_mean']:.4f}, std={stats['coarse_cd_std']:.4f}")
            logger.info(f"    Coarse Cl: mean={stats['coarse_cl_mean']:.4f}, std={stats['coarse_cl_std']:.4f}")
        
        # Verify coefficient data is present
        sample = train_dataset[0]
        if 'fine_coefficients' in sample:
            logger.info("  ✓ Coefficient data confirmed in dataset")
            logger.info(f"    Sample coefficients shape: {sample['fine_coefficients'].shape}")
            logger.info(f"    Coefficients present: Cd={sample['fine_coefficients'][0]:.4f}, Cl={sample['fine_coefficients'][1]:.4f}")
        else:
            logger.error("  ✗ Coefficient data NOT found in dataset samples!")
            logger.error("    Check coefficient CSV files and paths")
            raise ValueError("Coefficient data not found in dataset when predict_coefficients=True")
            
    else:
        # Use standard dataset (original code)
        logger.info("Using standard dataset without coefficient data")
    ...
    # Create data loaders (same for both dataset types)
    ...
    # Model creation - use V3 model if coefficient prediction is enabled
    if use_enhanced_features:
        # Check if coefficient prediction is enabled
        ...
        if predict_coefficients:
            ...
            logger.info("    - Surface field prediction: ENABLED")
            logger.info("    - Coefficient prediction: ENABLED")
            logger.info("    - Multi-task learning: ACTIVE")
        else:
            ...
            logger.info("  Enhanced model created (without coefficient prediction)")
    else:
    ...
    if use_physics_loss:
        ...
        logger.info(f"Using learning rate: {initial_lr} (adjusted for physics-aware training)")
    else:
        initial_lr = 0.001
        logger.info(f"Using learning rate: {initial_lr}")
    ...
    best_coeff_error = float('inf')  # Track best coefficient prediction error
    ...
    if use_coefficient_data:
        logger.info("  Mode: Multi-task (surface fields + coefficients)")
    elif use_enhanced_features:
        logger.info("  Mode: Enhanced (coarse-to-fine)")
    else:
    ...
    for epoch in range(init_epoch, cfg.train.epochs):
        # [Keep all the existing training loop code unchanged]
        # The training loop logic doesn't need modification
        # as it already handles the different training modes
        ...
        if use_enhanced_features:
            if use_physics_loss:
                ...
                avg_loss = train_epoch_enhanced_physics(
                    ...
                    cfg=cfg,  # Pass config for multi-task loss
                )
        ...
        # [Rest of the training loop continues unchanged...]
